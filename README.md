...
0. (not scripted)                 Cohort clinical/participant metadata --
                                   ...
                                   de_meta_after_preprocess.tsv -- must be
                                   sourced from the original Callahan et al.
                                   2017 public dataset and the Borum et al.
                                   2025 trial data respectively. These are
                                   not derived from sequencing and cannot be
                                   regenerated by any script in this repo;
                                   they need to be gathered/placed by hand.

1. 01_sequence_processing.sh      Metaxa2 archaeal screening (Sec. 2) +
                                   Lotus3/DADA2 ASV calling (Sec. 3.1).
                                   Uses Lotus3's own higherLvl/family.txt
                                   output directly -- no separate
                                   aggregation script needed.
                                   outputs: archaea.txt,
                                            us_counts_after_preprocess.tsv,
                                            de_counts_after_preprocess.tsv
                                   NOTE: sample manifests and paths are
                                   hard-coded in this script, not passed
                                   as config -- edit the literal values
...
`01_sequence_processing.sh`, `aggregate_to_family.py`, `permanova_maaslin2.R`,
and the bug fixes in `preprocess.py` were reconstructed from the manuscript's
Methods section to fill gaps in this repository. They have not been run
against the original raw data, so they are not yet verified to reproduce the
paper's exact reported numbers. Before treating them as final:
...
or the exact taxonomy rank column names in `aggregate_to_family.py` --
both are called out inline in those files.
