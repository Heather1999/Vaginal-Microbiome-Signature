...
meta_df["Preterm"] = np.where(
    meta_df["Preterm"].astype(str).str.startswith(("P", "1")), 1, 0
)
meta_df = meta_df.set_index("Unnamed: 0")

# Join taxa and cohort info
X_df = taxa_df.join(meta_df[["cohort", "PatID"]])
y = meta_df["Preterm"].astype(int).values
...
axes[0].set_xlabel("False Positive Rate (Positive label: Preterm)")
axes[0].set_ylabel("True Positive Rate (Positive label: Preterm)")
...
axes[1].set_xlabel("Recall (Positive label: Preterm)")
axes[1].set_ylabel("Precision (Positive label: Preterm)")
plt.tight_layout()
plt.savefig("birth_model_report.pdf", dpi=300, bbox_inches='tight', transparent=True)
plt.show()
...
selected.to_csv("Strict_US_trained_Features.tsv", sep='\t')